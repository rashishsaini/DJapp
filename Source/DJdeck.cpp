...
//==============================================================================
DJdeck::DJdeck(juce::AudioFormatManager& formatManagerToUse) 
...
//==============================================================================
void DJdeck::prepareToPlay (int samplesPerBlockExpected, double sampleRate)
{
    transportSource.prepareToPlay(samplesPerBlockExpected, sampleRate);
}

void DJdeck::releaseResources()
{
    transportSource.releaseResources();
}

void DJdeck::changeState(TransportState newState)
    {
        if (state != newState)
        {
            state = newState;
...
void DJdeck::getNextAudioBlock (const juce::AudioSourceChannelInfo& bufferToFill)
{
...
void DJdeck::changeListenerCallback (juce::ChangeBroadcaster* source)
// {
...
void DJdeck::transportSourceChanged()
{
    if (transportSource.isPlaying())
        changeState(Playing);
    else if ((state == Stopping) || (state == Playing))
...
void DJdeck::paint (juce::Graphics& g)
{
...
void DJdeck::paintIfNoFileLoaded (juce::Graphics& g, juce::Rectangle<int>& thumbnailBounds)
{
...
void DJdeck::paintIfFileLoaded (juce::Graphics& g, juce::Rectangle<int>& thumbnailBounds)
{
    g.setColour (juce::Colours::white);
    g.fillRect (thumbnailBounds);
