...
void MainComponent::prepareToPlay(int samplesPerBlockExpected, double sampleRate)
{
    deckA.prepareToPlay(samplesPerBlockExpected, sampleRate);
    deckB.prepareToPlay(samplesPerBlockExpected, sampleRate);
}
void MainComponent::getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill)
{
...
void MainComponent::paint(juce::Graphics& g)
{
    ...
    g.setColour(juce::Colours::white);
    g.fillRect(playlistBounds);
    g.setColour(juce::Colours::grey);
    g.drawFittedText("Playlist", playlistBounds, juce::Justification::centred, 1);
}
void MainComponent::resized()
{
    int deckHeight= 400;
    int halfWidth = getWidth() / 2;
    deckA.setBounds(0, 0, halfWidth, deckHeight);
    deckB.setBounds(halfWidth, 0, halfWidth, deckHeight);
    playlistBounds.setBounds(0, deckHeight, getWidth(), getHeight() - deckHeight);
}